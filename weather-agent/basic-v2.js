...
function getParsedJson(jsonStr) {
  try {
...
while (initialIt < maxIt) {
  console.log("--- Iteration:", initialIt + 1, "---");
  ...
  messages.push({ role: "developer", content: output }); // auto prompting

  const parsedOutput = getParsedJson(output);

  if (!parsedOutput) {
    // Retry this step again
    continue;
  }

  if (parsedOutput.type === "OUTPUT") {
